...
const dom = {
    ...
    // 分段与周期
    btnPeriods: document.querySelectorAll('.btn-period'),
    btnMeals: document.querySelectorAll('.btn-meal'),
    weekChips: document.querySelectorAll('.week-chip'),
...
function attachGlobalListeners() {
    ...
    // 分段选择互斥（改为多选统一风格）
    dom.btnPeriods.forEach(btn => {
        btn.addEventListener('click', () => toggleChip(btn, 'bg-macaroon-pink', 'text-white', 'border-transparent'));
    });
    dom.btnMeals.forEach(btn => {
        btn.addEventListener('click', () => toggleChip(btn, 'bg-macaroon-pink', 'text-white', 'border-transparent'));
    });

    // 星期全选与多选
    dom.weekChips.forEach(c => {
        c.addEventListener('click', () => toggleChip(c, 'bg-gray-800', 'text-white', 'border-gray-800'));
    });
...
// 辅助功能：让胶囊可 Toggle 并改变风格
function toggleChip(btn, activeBg, activeTxt, activeBorder) {
    if (btn.classList.contains(activeBg)) {
        btn.classList.remove(activeBg, activeTxt, activeBorder, 'font-bold');
        btn.classList.add('text-gray-600', 'border-gray-200');
    } else {
        btn.classList.remove('text-gray-600', 'border-gray-200');
        btn.classList.add(activeBg, activeTxt, activeBorder, 'font-bold');
    }
}
...
function fillSheetWithRule(ruleObj, isCopy = false) {
    ...
    if (ruleObj.period && Array.isArray(ruleObj.period)) {
        ruleObj.period.forEach(val => {
            let btn = Array.from(dom.btnPeriods).find(b => b.dataset.val === val);
    ...
    if (ruleObj.meal && Array.isArray(ruleObj.meal)) {
        ruleObj.meal.forEach(val => {
            let btn = Array.from(dom.btnMeals).find(b => b.dataset.val === val);
            if (btn) btn.classList.add('bg-macaroon-pink', 'text-white', 'border-transparent', 'font-bold');
        });
    }
