...
(() => {
    ...
    const dates = Array.from(html.matchAll(/data-date="([^"]+)"/g)).map(match => match[1]);
    assert.deepEqual(
        dates,
        [
            '2026-05-04',
        ...
        'Week view should render only the selected Monday-Sunday row'
    );
})();
...
(() => {
    ...
    calendar.selectedDate = '2026-05-06';
    assert.equal(calendar.getToggleButtonText(), '显示当周');
    assert.equal(calendar.toggleViewMode(), 'week');
    assert.equal(calendar.getToggleButtonText(), '显示当月');

    const html = calendar.generateCalendarHTML(2026, 4, {});
    const dates = Array.from(html.matchAll(/data-date="([^"]+)"/g)).map(match => match[1]);
    assert.equal(dates.length, 7, 'Toggling to week view should collapse the rendered range');
})();
